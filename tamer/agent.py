...
class SGDFunctionApproximator:
    ...
    def featurize_state(self, state):
        array_state = np.asarray(state[0])  # Convert the state to a NumPy array
        array_state = np.atleast_2d(array_state)  # Ensure array_state is at least 2-dimensional
        if array_state.shape[1] == 1:
            array_state = np.concatenate((array_state, np.zeros((array_state.shape[0], 1))), axis=1)
        scaled = self.scaler.transform(array_state)  # Apply scaling to array_state
        featurized = self.featurizer.transform(scaled)  # Apply featurization to the scaled array

        return featurized[0] 
    
        print(state)
        array_state = state[0]
        array_state = np.atleast_2d(array_state)
        # array_state = np.reshape(array_state, (1, -1))  # Reshape the input array
        scaled = self.scaler.transform(array_state)
    ...
    def _train_episode(self, episode_index, disp):
        ...
        with open(self.reward_log_path, 'a+', newline='') as write_obj:
            ...
            for ts in count():
                ...
                if self.tame:
                    print(MOUNTAINCAR_ACTION_MAP[action])
                ...
                if not self.tame:
                    ...
                    while time.time() < now + self.ts_len:
                        ...
                        # TODO: Add human feedback to train the agent

                        feedback_ts = dt.datetime.now().time()
                        
                        # check if there is human reward and complete the code snippet
                            # dict_writer.writerow(
                            #     {
                            #         'Episode': episode_index + 1,
                            #         'Ep start ts': ep_start_time,
                            #         'Feedback ts': feedback_ts,
                            #         'Human Reward': #provide the human reward
                            #         'Environment Reward': reward
                            #     }
                            # )
                            # self.H.update(state, action) #update human reward
                            # break
